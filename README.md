...
| 來源 | 新鮮度 | 有沒有日期 |
| --- | --- | --- |
| e添富 `close1` | 快（收盤後很快就換日） | ❌ 沒有 |
| 每日行情 `STOCK_DAY_ALL.ClosingPrice` | 慢（實測 2026-09-09 17:40 仍停在 9/8） | ❌ 沒有，但與有日期的日報同批發布 |

實測 2026-09-09 17:40：櫃買已經發布 9/9 行情，證交所日報與每日行情卻還停在 9/8，e添富 則已經是 9/9 的收盤——234 檔中有 223 檔對不上。只用日行情會讓上市價格整整慢一天（上櫃卻是當天），只用 e添富 又沒有日期可標。因此改成比對兩者：
...
上櫃則直接用櫃買行情每列自帶的 `Date`，逐檔各標各的。每一檔實際採用的日期都寫在該檔的 `priceDate`，畫面完全以它為準；`meta.listedReportDate` 另外保留證交所日報自己的交易日，方便追查兩邊差幾天。

健康度警告（只警告、不中止，會寫進 `syncWarning` 顯示在畫面上）：e添富 領先時說明改用了哪個來源與日期依據；當日行情覆蓋率低於 90% 時也會提示。
